...
function categoryPanel(c,cat){
  ...
  if(!products.length) e.addFields({name:"No products yet",value:"This department has no products available right now. Check back soon.",inline:false});
...
function nav(gid,tab){return ["overview","shop","payments","orders","tickets","security","settings","logs"].map(x=>`<a class="${tab===x?"active":""}" href="/dashboard?guild=${encodeURIComponent(gid)}&tab=${x}">${x.toUpperCase()}</a>`).join("");}
