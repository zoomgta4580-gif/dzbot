...
const BOT_NAME = process.env.BOT_NAME || "DZSERVICE";
...
function parseEmoji(v){
  ...
  if(s && s.length<=8 && !/[a-zA-Z0-9:<>]/.test(s)) return s;
  return null;
}
function safeEmoji(v,fallback){ return parseEmoji(v)||fallback; }
function safeUrl(v){ try { const u = new URL(String(v)); return ["http:","https:"].includes(u.protocol) ? u.toString() : ""; } catch { return ""; } }
function parseFeatures(v){ return String(v||"").split("\n").map(x=>cut(x.trim(),150)).filter(Boolean).slice(0,15); }
function defaults(){
  return {
    brandColor: DEFAULT_COLOR, shopTitle:"🛒 DZSERVICE",
    shopDescription:"Explore our departments, choose a product, and place an order in seconds.",
    verifyTitle:"🔐 VERIFY YOUR ACCOUNT", verifyDescription:"Complete verification to unlock the server.", verifyButtonLabel:"VERIFY NOW",
...
function cfg(gid){
  ...
  for(const cat of c.categories){ if(!Array.isArray(cat.products)) cat.products=[]; if(cat.enabled===undefined) cat.enabled=true; if(!Array.isArray(cat.features)) cat.features=[]; if(cat.websiteUrl===undefined) cat.websiteUrl=""; if(cat.showcaseUrl===undefined) cat.showcaseUrl=""; }
  return c;
}
...
function embed(c,title,description){
  const e=new EmbedBuilder().setColor(hex(c.brandColor)).setDescription(cut(description,4096)).setFooter({text:`${BOT_NAME} • Premium Commerce System`}).setTimestamp();
  if(title) e.setTitle(cut(title,256));
  return e;
}
...
function verifyPanel(c){
  return {embeds:[bigPanel(c,c.verifyTitle,`${c.verifyDescription}\n\n**Secure • Fast • One Click**\n\nClick the button below to receive the configured verification role.`)],
    components:[new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("verify").setLabel(cut(c.verifyButtonLabel||"VERIFY NOW",80)).setEmoji("✅").setStyle(ButtonStyle.Success))]};
}
...
function shopMenu(c){
  ...
  menu.addOptions(cats.map(x=>({label:cut(x.name,100),value:x.id,description:cut(x.description||"Browse products",100),emoji:safeEmoji(x.emoji,"📦")})));
...
function categoryPanel(c,cat){
  ...
  const lines=[`__**${cat.emoji||"📦"} ${cat.name}**__`,""];
  if(cat.description) lines.push(cat.description,"");
  if(cat.features.length){ for(const f of cat.features) lines.push(`✅ ${f}`); lines.push(""); }
  lines.push("**Prices:**","");
  if(products.length) for(const p of products) lines.push(`${p.emoji||"✦"}${p.name} = ${p.price}${p.description?`\n${p.description}`:""}`,"");
  else lines.push("_No products available right now. Check back soon._","");
  if(cat.websiteUrl) lines.push(`Website --> [here](${cat.websiteUrl})`);
  if(cat.showcaseUrl) lines.push(`Showcase --> [here](${cat.showcaseUrl})`);
  const e=embed(c,null,lines.join("\n").trim());
  if(cat.image) e.setImage(cat.image);
  const select=new StringSelectMenuBuilder().setCustomId(`product_select:${cat.id}`).setPlaceholder("🛍️ Select a product");
  if(products.length) select.addOptions(products.map(p=>({label:cut(p.name,100),value:p.id,description:cut(`${p.price} • ${p.description||"Order now"}`,100),emoji:safeEmoji(p.emoji,"🛍️")})));
...
async function openOrderTicket(i,o){
  const c=cfg(i.guild.id), me=i.guild.members.me;
  const overwrites=[
  ...
  const ch=await i.guild.channels.create({name:`order-${i.user.username.replace(/[^a-z0-9]/gi,"-").slice(0,15)}-${Date.now().toString().slice(-4)}`,
    type:ChannelType.GuildText,parent:c.orderCategoryId||c.ticketCategoryId||undefined,permissionOverwrites:overwrites,topic:`Order ${o.number} for ${i.user.tag}`});
  o.channelId=ch.id;
  await ch.send({content:`<@${i.user.id}>${c.ticketSupportRoleId?` <@&${c.ticketSupportRoleId}>`:""}`,embeds:[orderEmbed(c,o)],components:[orderButtons(o.id)]});
  c.tickets.push({id:uid(),channelId:ch.id,userId:i.user.id,userName:i.user.tag,createdAt:Date.now(),orderId:o.id});
  log(i.guild.id,"Order ticket opened",i.user,{orderId:o.number,channelId:ch.id});
  return ch;
}
...
client.on("interactionCreate",async i=>{
  try{
    ...
    if(i.isModalSubmit()&&i.customId.startsWith("checkout:")){
      ...
      const ch=await openOrderTicket(i,o); save();
      return i.editReply({content:`✅ Order **${o.number}** created. A private order channel has been opened for you: <#${ch.id}>`});
    }
    ...
    if(i.commandName==="setup"){
      ...
      if(text&&!c.shopTitle)c.shopTitle="🛒 DZSERVICE";
...
function nav(gid,tab){return ["overview","shop","payments","tickets","security","settings","logs"].map(x=>`<a class="${tab===x?"active":""}" href="/dashboard?guild=${encodeURIComponent(gid)}&tab=${x}">${x.toUpperCase()}</a>`).join("");}
function dashboardHTML(g,c,guilds,channels,roles,req){
  ...
  if(tab==="shop") content=`<div class="card"><h2>🛒 Interactive Shop</h2><p>Create departments, products, images and pricing. Customers use dropdown menus directly in Discord.</p>${c.categories.map(cat=>`<div class="cat"><div class="cathead"><h3>${esc(cat.emoji)} ${esc(cat.name)}</h3><span>${cat.enabled===false?"DISABLED":"ACTIVE"}</span></div><p>${esc(cat.description)}</p><p class="muted">${cat.products.length} products · ${cat.channelId?`<b>#${esc(channels.find(x=>x.id===cat.channelId)?.name||"unknown")}</b>`:"⚠️ No channel set — sending will fail"}</p><details><summary style="cursor:pointer;color:#8799ff;font-weight:800;margin:6px 0">✏️ EDIT DEPARTMENT</summary><form method="post" action="/category/edit" class="new" style="margin-top:10px"><input type="hidden" name="_csrf" value="${token}"><input type="hidden" name="guild" value="${g.id}"><input type="hidden" name="categoryId" value="${cat.id}"><div class="row"><input name="emoji" value="${esc(cat.emoji)}"><input name="name" value="${esc(cat.name)}" required><select name="channelId">${selectOptions(channels,cat.channelId,"Select Discord channel")}</select><input name="image" value="${esc(cat.image||"")}" placeholder="Image URL"></div><textarea name="description" placeholder="Department description">${esc(cat.description)}</textarea><textarea name="features" placeholder="Feature list, one per line — shown with ✅ checkmarks">${esc((cat.features||[]).join("\n"))}</textarea><div class="row"><input name="websiteUrl" value="${esc(cat.websiteUrl||"")}" placeholder="Website URL (optional)"><input name="showcaseUrl" value="${esc(cat.showcaseUrl||"")}" placeholder="Showcase URL (optional)"></div><button class="btn">SAVE CHANGES</button></form></details>${cat.products.map(p=>`<div class="item"><span><b>${esc(p.emoji)} ${esc(p.name)}</b><small>${esc(p.price)} · ${esc(p.description)}</small></span><form method="post" action="/product/delete"><input type="hidden" name="_csrf" value="${token}"><input type="hidden" name="guild" value="${g.id}"><input type="hidden" name="categoryId" value="${cat.id}"><input type="hidden" name="productId" value="${p.id}"><button class="danger">DELETE</button></form></div>`).join("")}<form method="post" action="/product"><input type="hidden" name="_csrf" value="${token}"><input type="hidden" name="guild" value="${g.id}"><input type="hidden" name="categoryId" value="${cat.id}"><div class="row"><input name="name" placeholder="Product name" required><input name="price" placeholder="Price" required><input name="emoji" value="✦"><input name="description" placeholder="Description"></div><button class="btn">+ ADD PRODUCT</button></form><div class="actions"><form method="post" action="/send-category"><input type="hidden" name="_csrf" value="${token}"><input type="hidden" name="guild" value="${g.id}"><input type="hidden" name="categoryId" value="${cat.id}"><button class="btn">📤 SEND DEPARTMENT</button></form><form method="post" action="/category/toggle"><input type="hidden" name="_csrf" value="${token}"><input type="hidden" name="guild" value="${g.id}"><input type="hidden" name="categoryId" value="${cat.id}"><button class="btn ghost">${cat.enabled===false?"ENABLE":"DISABLE"}</button></form><form method="post" action="/category/delete"><input type="hidden" name="_csrf" value="${token}"><input type="hidden" name="guild" value="${g.id}"><input type="hidden" name="categoryId" value="${cat.id}"><button class="danger">DELETE DEPARTMENT</button></form></div></div>`).join("")}
  <form method="post" action="/category" class="new"><input type="hidden" name="_csrf" value="${token}"><input type="hidden" name="guild" value="${g.id}"><h3>➕ CREATE DEPARTMENT</h3><div class="row"><input name="emoji" value="📦"><input name="name" placeholder="Department name" required><select name="channelId">${selectOptions(channels,"","Select Discord channel")}</select><input name="image" placeholder="Image URL"></div><textarea name="description" placeholder="Department description"></textarea><textarea name="features" placeholder="Feature list, one per line — shown with ✅ checkmarks"></textarea><div class="row"><input name="websiteUrl" placeholder="Website URL (optional)"><input name="showcaseUrl" placeholder="Showcase URL (optional)"></div><button class="btn">CREATE DEPARTMENT</button></form></div>`;
  if(tab==="payments") content=`<div class="card"><h2>💳 Payment Methods</h2><p>These payment methods are shown to customers on the Payments panel (sent via "Send Payments" or <code>/payments</code>). ${c.paymentChannelId?`Panel channel: <b>#${esc(channels.find(x=>x.id===c.paymentChannelId)?.name||"unknown")}</b>`:"⚠️ No payment channel set yet — configure it in Settings."}</p>${c.payments.map(p=>`<div class="item"><span><b>${esc(p.emoji)} ${esc(p.name)}</b><small>${esc(p.description)}</small></span><form method="post" action="/payment/delete"><input type="hidden" name="_csrf" value="${token}"><input type="hidden" name="guild" value="${g.id}"><input type="hidden" name="paymentId" value="${p.id}"><button class="danger">DELETE</button></form></div>`).join("")||"<p class='muted'>No payment methods yet — add one below.</p>"}
  <form method="post" action="/payment/add" class="new"><input type="hidden" name="_csrf" value="${token}"><input type="hidden" name="guild" value="${g.id}"><h3>➕ ADD PAYMENT METHOD</h3><div class="row"><input name="emoji" value="💳"><input name="name" placeholder="e.g. PayPal, Bank Transfer, Crypto" required><input name="description" placeholder="Details shown to customers (e.g. account/address)" style="grid-column:span 2"></div><button class="btn">ADD PAYMENT METHOD</button></form>
  <div class="actions" style="margin-top:14px"><form method="post" action="/send"><input type="hidden" name="_csrf" value="${token}"><input type="hidden" name="guild" value="${g.id}"><input type="hidden" name="type" value="payments"><button class="btn">📤 SEND PAYMENTS PANEL</button></form></div></div>`;
  ...
  if(tab==="settings") content=`<div class="card"><h2>⚙️ Server Configuration</h2><form method="post" action="/settings"><input type="hidden" name="_csrf" value="${token}"><input type="hidden" name="guild" value="${g.id}"><div class="grid2"><label>Shop title<input name="shopTitle" value="${esc(c.shopTitle)}"></label><label>Brand color<input name="brandColor" value="${esc(c.brandColor)}"></label><label>Shop description<textarea name="shopDescription">${esc(c.shopDescription)}</textarea></label><label>Verify title<input name="verifyTitle" value="${esc(c.verifyTitle)}"></label><label>Verify description<textarea name="verifyDescription">${esc(c.verifyDescription)}</textarea></label><label>Verify button text<input name="verifyButtonLabel" value="${esc(c.verifyButtonLabel)}" maxlength="80" placeholder="VERIFY NOW"></label><label>Ticket title<input name="ticketTitle" value="${esc(c.ticketTitle)}"></label><label>Ticket description<textarea name="ticketDescription">${esc(c.ticketDescription)}</textarea></label><label>Payment title<input name="paymentTitle" value="${esc(c.paymentTitle)}"></label><label>Payment description<textarea name="paymentDescription">${esc(c.paymentDescription)}</textarea></label></div><h3>Discord Channels & Roles</h3><div class="grid2"><label>Verify channel<select name="verifyChannelId">${selectOptions(channels,c.verifyChannelId, "Select channel")}</select></label><label>Verify role<select name="verifyRoleId">${selectOptions(roles,c.verifyRoleId,"Select role")}</select></label><label>Payment channel<select name="paymentChannelId">${selectOptions(channels,c.paymentChannelId,"Select channel")}</select></label><label>Ticket panel channel<select name="ticketPanelChannelId">${selectOptions(channels,c.ticketPanelChannelId,"Select channel")}</select></label><label>Ticket category<select name="ticketCategoryId">${selectOptions(channels.filter(x=>x.type==="category"),c.ticketCategoryId,"Select category")}</select></label><label>Ticket support role<select name="ticketSupportRoleId">${selectOptions(roles,c.ticketSupportRoleId,"Select role")}</select></label><label>Order channel<select name="orderChannelId">${selectOptions(channels,c.orderChannelId,"Select channel")}</select></label><label>Order ticket category<select name="orderCategoryId">${selectOptions(channels.filter(x=>x.type==="category"),c.orderCategoryId,"Select category (optional, falls back to ticket category)")}</select></label></div><button class="btn">SAVE ALL SETTINGS</button></form></div>`;
...
app.post("/settings",auth,(req,res)=>{if(!postOK(req,res))return;const c=cfg(req.body.guild);Object.assign(c,{shopTitle:cut(req.body.shopTitle||c.shopTitle,256),shopDescription:cut(req.body.shopDescription||c.shopDescription,4000),brandColor:hex(req.body.brandColor),verifyTitle:cut(req.body.verifyTitle||c.verifyTitle,256),verifyDescription:cut(req.body.verifyDescription||c.verifyDescription,4000),verifyButtonLabel:cut(req.body.verifyButtonLabel||c.verifyButtonLabel,80),ticketTitle:cut(req.body.ticketTitle||c.ticketTitle,256),ticketDescription:cut(req.body.ticketDescription||c.ticketDescription,4000),paymentTitle:cut(req.body.paymentTitle||c.paymentTitle,256),paymentDescription:cut(req.body.paymentDescription||c.paymentDescription,4000),verifyChannelId:req.body.verifyChannelId||null,verifyRoleId:req.body.verifyRoleId||null,paymentChannelId:req.body.paymentChannelId||null,ticketPanelChannelId:req.body.ticketPanelChannelId||null,ticketCategoryId:req.body.ticketCategoryId||null,ticketSupportRoleId:req.body.ticketSupportRoleId||null,orderChannelId:req.body.orderChannelId||null,orderCategoryId:req.body.orderCategoryId||null});save();log(req.body.guild,"Dashboard settings updated",req.session.user);redirectDash(res,req.body.guild,"settings");});
app.post("/category",auth,(req,res)=>{if(!postOK(req,res))return;cfg(req.body.guild).categories.push({id:uid(),name:cut(req.body.name,200),emoji:cut(req.body.emoji||"📦",10),description:cut(req.body.description||"",2000),channelId:req.body.channelId||null,image:safeUrl(req.body.image),features:parseFeatures(req.body.features),websiteUrl:safeUrl(req.body.websiteUrl),showcaseUrl:safeUrl(req.body.showcaseUrl),enabled:true,products:[]});save();log(req.body.guild,"Department created",req.session.user);redirectDash(res,req.body.guild,"shop");});
app.post("/category/edit",auth,(req,res)=>{if(!postOK(req,res))return;const cat=cfg(req.body.guild).categories.find(x=>x.id===req.body.categoryId);if(!cat)return res.status(404).send("Department not found");cat.name=cut(req.body.name||cat.name,200);cat.emoji=cut(req.body.emoji||cat.emoji,10);cat.description=cut(req.body.description||"",2000);cat.image=safeUrl(req.body.image);cat.channelId=req.body.channelId||null;cat.features=parseFeatures(req.body.features);cat.websiteUrl=safeUrl(req.body.websiteUrl);cat.showcaseUrl=safeUrl(req.body.showcaseUrl);save();log(req.body.guild,"Department updated",req.session.user);redirectDash(res,req.body.guild,"shop");});
...
app.post("/payment/add",auth,(req,res)=>{if(!postOK(req,res))return;const c=cfg(req.body.guild);if(!req.body.name)return res.status(400).send("Payment name is required.");c.payments.push({id:uid(),name:cut(req.body.name,100),emoji:cut(req.body.emoji||"💳",10),description:cut(req.body.description||"",500)});save();log(req.body.guild,"Payment method added",req.session.user,{name:req.body.name});redirectDash(res,req.body.guild,"payments");});
app.post("/payment/delete",auth,(req,res)=>{if(!postOK(req,res))return;const c=cfg(req.body.guild);c.payments=c.payments.filter(x=>x.id!==req.body.paymentId);save();log(req.body.guild,"Payment method removed",req.session.user);redirectDash(res,req.body.guild,"payments");});
...
app.post("/send",auth,async(req,res)=>{if(!postOK(req,res))return;const c=cfg(req.body.guild),type=req.body.type;let chId,payload;
  ...
  const ch=client.channels.cache.get(chId);if(!ch?.isTextBased())return res.status(400).send(page("Send Error","<div class='auth'><div class='hero'><h1>❌ Channel Not Configured</h1><p>Select a valid channel in Settings, then try again.</p><a class='btn' href='/dashboard?guild="+encodeURIComponent(req.body.guild)+"&tab=settings'>Go to Settings</a></div></div>"));
  await ch.send(payload);log(req.body.guild,`${type} panel sent`,req.session.user);redirectDash(res,req.body.guild,"overview");
});
app.post("/send-category",auth,async(req,res)=>{if(!postOK(req,res))return;const c=cfg(req.body.guild),cat=c.categories.find(x=>x.id===req.body.categoryId);if(!cat)return res.status(404).send("Department not found.");const ch=client.channels.cache.get(cat.channelId);if(!ch?.isTextBased())return res.status(400).send(page("Send Error","<div class='auth'><div class='hero'><h1>❌ No Channel Set For This Department</h1><p>Open the Shop tab, pick a channel for \""+esc(cat.name)+"\" under Edit Department, save, then try Send Department again.</p><a class='btn' href='/dashboard?guild="+encodeURIComponent(req.body.guild)+"&tab=shop'>Back to Shop</a></div></div>"));await ch.send(categoryPanel(c,cat));log(req.body.guild,"Department panel sent",req.session.user);redirectDash(res,req.body.guild,"shop");});
app.use((req,res)=>res.status(404).send(page("404","<div class='auth'><div class='hero'><h1>404</h1><p>Page not found.</p></div></div>")));
app.use((err,req,res,next)=>{console.error("❌ Web error:",err);if(!res.headersSent)res.status(500).send(page("Error",`<div class='auth'><div class='hero'><h1>⚠️ Server Error</h1><p>${esc(err?.message||"Unknown error")}</p><a class="btn ghost" href="javascript:history.back()">Go Back</a></div></div>`));});
